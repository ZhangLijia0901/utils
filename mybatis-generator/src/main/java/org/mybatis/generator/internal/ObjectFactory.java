...
public class ObjectFactory {
	private static Map<DataBaseType, ClassLoader> externalClassLoaders;
	private static List<ClassLoader> resourceClassLoaders;

	static {
		externalClassLoaders = new HashMap<DataBaseType, ClassLoader>();
	 ...
	 * resources. Currently, this is only used when searching for properties files
	 * that may be referenced in the configuration file.
	 * 
	 * @param classLoader
	 */
	 ...
	 * @param classLoader
	 */
	public static synchronized void addExternalClassLoader(Map<DataBaseType, ClassLoader> classLoader) {
		ObjectFactory.externalClassLoaders.putAll(classLoader);
	}

	public static ClassLoader getExternalClassLoader(DataBaseType dataBaseType) {
		return ObjectFactory.externalClassLoaders.get(dataBaseType);
	}

	/**
	 * This method returns a class loaded from the context classloader, or the
	 * classloader supplied by a client. This is appropriate for JDBC drivers, model
	 * root classes, etc. It is not appropriate for any class that extends one of
	 ...
	 */
	public static Class<?> externalClassForName(DataBaseType baseType, String type) throws ClassNotFoundException {

		Class<?> clazz;
		if (baseType != null)
			return Class.forName(type, true, externalClassLoaders.get(baseType));

		for (ClassLoader classLoader : externalClassLoaders.values()) {
			try {
	...
	public static Object createExternalObject(String type) {
		...
		try {
			Class<?> clazz = externalClassForName(null, type);
	...
	/**
	 * This method creates an introspected table implementation that is only usable
	 * for validation (i.e. for a context to determine if the target is ibatis2 or
	 * mybatis3).
	 * 
	 * @param context
	 * @return
